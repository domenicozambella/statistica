...
%%%%%%%%%%%%
%%%%%%%%%%%%
\hfill\rlap{.}\clearpage\section{Intervallo di confidenza}
\label{IC}

\def\ceq#1#2#3{\parbox{20ex}{$\displaystyle #1$}\medrel{#2}$\displaystyle  #3$}

\subsection{Intervallo di confidenza per normale con varianza nota}
\label{IC_varianza_nota}

Consideriamo v.a.\@ $X\sim N(\mu,\sigma^2)$ con $\sigma$ nota e $\mu$ ignota.  Sia $\bar X$ la media campionaria. Fissiamo un $\epsilon>0$.

La probabilit\`a che $\bar X$ sia a distanza inferiore a $\epsilon$ da $\mu$ è
...
\ceq{}{=}{1-\alpha}\hfill (questa è una definizione di $\alpha$)
...
L'interpretazione `e che se ripetiamo l'esperimento, con probabilità $1-\alpha$ ritroveremo un intervallo che contiene $\mu$.
...
\item[4.] Il p-valore è $\Pr(T\ge t)$ dve $T\sim t(5)$. Ovvero {\tt pt($t$, $n-1$)}\hfill{\tt (0.074)}
...
\item[3.] $T$-test per due popolazioni a due code

\item[3.] Il $t$-score è 
$t = \dfrac{\bar x - \bar y}{\sqrt{s^2_x/n_x+s^2_y/n_y}}$ dove 

$\displaystyle\bar x = \frac1{n_x}\sum^6_{i=1}x_i$\hfil$\displaystyle\bar y = \frac1{n_y}\sum^6_{i=1}y_i$
...
\item[3.] Il $t$-score è 
$t = \dfrac{\bar z}{s/\sqrt{n}}$ dove $z_i=x_i-z_i$ e

$\displaystyle\bar z = \frac1{n}\sum^n_{i=1}z_i$

$\displaystyle s^2= \frac1{n}\sum^n_{i=1}z_i^2$
