...
\emph{$P(T_+|H_0)=\emph{$\alpha$}$}\hfill \emph{significatività\/}
}}
\kern1ex
\fbox{\parbox[t]{38ex}{%
\emph{$T_+\cap H_A$} \ \textit{corretto positivo}\\

\emph{$P(T_+|H_A)=1-\emph{$\beta$}$} \ \textit{sensibilità}\hfill \emph{potenza\/}
}}
...
\emph{$P(T_-|H_0)=1-\emph{$\alpha$}$} \ \textit{sepecificità}
}}
\kern1ex
\fbox{\parbox[t]{38ex}{%
\emph{$T_-\cap H_A$} \ \textit{falso negativo}\hfill \emph{errore II tipo\/}\\

\emph{$P(T_-|H_A)=\emph{$\beta$}$}
}}


\bigskip
N.B. È vacile progettare un test che minimizza una tra $P(T_+|H_0)$ o $P(T_-|H_A)$. In un caso estremo: se a prescindere dai dati rifiuta sempre $H_0$ avrà banalmente $P(T_-| H_0)=0$; invece un test che non rifiuta mai $H_0$ avrà  $P(T_+|H_0)=0$. La difficoltà nel progettare il test è trovare il giusto equilibrio tra i due errori.
...
Sia $W$ una statistica e sia $w$ il valore osservato. Diamo due definizioni equivalenti di \emph{p-valore\/} 


\begin{itemize}
\item Il p-value di $w$ è il minimo $\alpha$ che permentte di rigettare $H_0$.

\item Il p-value di $w$ è la probabilità di osservare un risultato tanto estremo quanto $w$, nel caso $H_0$ sia vera. 
\end{itemize}

La seconda definizione suona più semplice ma richiede alcune precisazioni. 


% Tipicamente $H_0$ prevede un certo valore $w_0$ per la statistica. Il p-valore è la probabilità 
% 
% \ceq{\hfill \emph{p-valore}}{=}{P\big(|W-w_0|>|w-w_0|\big)}
% 
% \ceq{}{=}{P\big(W<w_0-|w_0-w|\big) + P\big(W>w_0+|w-w_0|\big)}. 
% 
% Spesso si effettua una trasformazione di coordinate in modo tale da avere $w_0=0$. quindi l'espressione si semplifica in 
% 
% \ceq{}{=}{P\big(W<w\big) + P\big(W>w\big)}.
% 
% Inoltre a volte la 
...
%%%%%%%%%%%%
%%%%%%%%%%%%
\clearpage\section{Binomial test}
\subsection{Test (una coda)}\label{Bernoulli_test}
...
%%%%%%%%%%%%
%%%%%%%%%%%%
\clearpage\subsection{Test (una coda). Errore I tipo.}
...
%%%%%%%%%%%%
%%%%%%%%%%%%
\clearpage\subsection{Test (a  una coda). Errore II tipo.}
...
%%%%%%%%%%%%
%%%%%%%%%%%%
\clearpage\subsection{Test (due code).}
...
%%%%%%%%%%%%
%%%%%%%%%%%%
\clearpage\subsection{Test (due code). Errori I e II tipo.}
...
%%%%%%%%%%%%
%%%%%%%%%%%%
\clearpage\subsection{Test (due code) campione più ampio.}
...
Il test consiste nel misurare la pressione ad un campione di $n$ pazienti. 


di e di calcolare la media capionaria

\ceq{\hfill \bar X}{=}{\frac1n\sum_{i=1}^nX_i}

Dome $X_i$ è la v.a.\@ che mi dà la pressione dell'$i$-esimo pazione del campione. Rigetteremo $H_0$ se il valore ottenuto è suporiore ad un certo $x_\alpha$ che vogliamo fissarein modo che l'errore I tipo uguale ad $\alpha$ (supposto noto). Serve quindi uno $x_\alpha$ tale che $P(\bar X>x_\alpha)=\alpha$.
...
% 
% Vogliamo $x_\alpha$ tale che $P(Z>\dfrac{x_\alpha-\mu_0}{\sigma/\sqrt{n}})=\alpha$
