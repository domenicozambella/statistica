...
%%%%%%%%%%%%
%%%%%%%%%%%%
\hfill\rlap{.}\clearpage\section{Teorema di Bayes}
...
\ceq{\hfill\Pr(B|A)}{=}{\dfrac{\Pr(A|B)\cdot \Pr(B)}{\Pr(A)}}
\medrel{=}$\dfrac{\Pr(A|B)\cdot \Pr(B)}{\Pr(A|B)\Pr(B)+\Pr(A|\neg B)\Pr(\neg B)}$
...
%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%
\hfill\rlap{.}\clearpage\section{Variabili aleatorie binomiali}
...
%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%
\hfill\rlap{.}\clearpage\section{Valore atteso e varianza}
...
%%%%%%%%%%%%
%%%%%%%%%%%%
\hfill\rlap{.}\clearpage\hfill\textbf{Pobabilità totali (richiede valore atteso)}\subsection{Peso neonati}
...
%%%%%%%%%%%%
%%%%%%%%%%%%
\hfill\rlap{.}\clearpage\hfill\textbf{Pobabilità totali (richiede nozione di idipendenza)}\subsection{Modello di Hardy-Weinberg (1)}
\label{HW_totali1}
...
\item specie dipoide
\item specie monoica (??)
\item accoppiamento casuale (random mating)
\item le generazioni non si sovrapongono
\item nessuna selezione
\end{citemize}
...
Denotiamo con $aa$ l'evento \textit{ha genotipo $aa$ alla prima generazione}. Analgamente per gli altri genotipi. Denotiamo con $aa\-ab$, etc.\@ l'evento: i genitori hanno genotipo $aa\mbox{-}ab$. Analgamente per le altre coppe di genotipi.
...
%%%%%%%%%%%%
%%%%%%%%%%%%
\hfill\rlap{.}\clearpage\hfill\textbf{Pobabilità totali (richiede nozione di idipendenza)}\subsection{Equilibrio di Hardy-Weinberg (2)}

\def\ceq#1#2#3{\parbox{12ex}{$\displaystyle #1$}\medrel{#2}$\displaystyle  #3$}

È utile osservare che $q_{aa}$, $q_{bb}$, e $q_{ab}$ dipendono solo dalla frazione di copie dell'allele $a$ presenti nella popolazione. Se $n$ \`e il numero di individui, $2n$ è il numero totale di copie dei due alleli. La frazione di $a$ sul totale è (indichiamola con $p_a$)
...
È intuitivo che la frazione di copie dell'allele $a$ non cambia di generazione in generazione. Verifichiamolo numericamente usando la formula $\#$
...
%%%%%%%%%%%%
%%%%%%%%%%%%
\hfill\rlap{.}\clearpage\hfill\textbf{Regola di Bayes}\subsection{Hemophilia gene carrier }
\label{Hemophilia}
...
$\Omega$\hfill  set of women w\!/ mother carrier, father and husband unaffected, one son

$C$\hfill set of women that are carrier

$S_{na}$\hfill set of women w\!/ son not affected
...
%%%%%%%%%%%%
%%%%%%%%%%%%
\hfill\rlap{.}\clearpage\hfill\textbf{Regola di Bayes}
...
Nella pratica questo test è spesso sostituito da un test sulle proporzioni ($Z$-test). Comunque in questa versione è concettualmente più semplice. 
...
%%%%%%%%%%%%
%%%%%%%%%%%%
\hfill\rlap{.}\clearpage\hfill\textbf{Test Binomiale}\subsection{Una coda, errore I tipo}
...
%%%%%%%%%%%%
%%%%%%%%%%%%
\hfill\rlap{.}\clearpage\hfill\textbf{Test Binomiale}\subsection{Testa una coda, errore II tipo}
